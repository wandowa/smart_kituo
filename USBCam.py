...
parser.add_argument('--camera_index', type=int, default=-1, help='USB webcam index (e.g., 0, 1, 2; default -1 to auto-detect)')
...
def initialize_camera():
    # Try user-specified camera index first, if provided
    indices = [args.camera_index] if args.camera_index >= 0 else [0, 1, 2, 3, 4]
    for index in indices:
        cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)  # CAP_DSHOW for Windows USB webcams
        if cap.isOpened():
            ...
            print(f"USB webcam initialized on index {index}")
            return cap, index
        cap.release()
        time.sleep(1)
    print(f"Error: No USB webcam found on indices {indices}.")
    return None, -1
...
def detect_and_count():
    ...
    try:
        while not stop_event.is_set():
            ret, frame = cap.read()
            if not ret:
                print(f"Warning: Frame failed on index {camera_index}.")
                retry_count += 1
            ...
            try:
                ...
                if frame_counter % 2 == 0:
                    cv2.imshow('USB Webcam - Passenger Counter', display_frame)
